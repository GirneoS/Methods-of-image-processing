...
def trace_path(
    ...
    # Флаг: предыдущий отскок был зеркальным (или это первый луч с камеры).
    # Emission добавляем только в этих случаях — иначе будет двойной счёт
    # (прямое освещение уже учтено через NEE).
    last_specular = True
    ...
    for depth in range(MAX_DEPTH):
        ...
        # Emission считаем только если пришли с камеры или после зеркального отскока,
        # потому что для диффузных поверхностей свет уже учтён через _direct_light (NEE).
        if last_specular:
        ...
        if event == "diffuse":
            ...
            last_specular = True  # после зеркала NEE не делался — emission надо считать
