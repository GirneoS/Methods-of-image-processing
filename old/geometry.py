...
@dataclass
class PointLight:
    """Точечный источник света: задаётся позицией и интенсивностью (RGB, Вт/стерадиан).

    В отличие от площадного источника (Triangle + emission) не имеет нормали и площади.
    Излучает равномерно во все стороны. Полная мощность: 4π · mean(intensity).
    """
    position: np.ndarray
    intensity: np.ndarray  # RGB Вт/ср
